...
const FISH_AUDIO_MODELS = ["s2-pro", "s1"];
...
function isValidFishAudioVoiceId(voiceId) {
  return /^[a-zA-Z0-9]{20,64}$/.test(voiceId);
}
function normalizeFishAudioProviderConfig(rawConfig) {
  ...
  return {
    ...
    resolveTalkConfig: ({ baseTtsConfig, talkProviderConfig }) => {
      ...
      return {
        ...
        ...trimToUndefined(talkProviderConfig.modelId) == null ? {} : { model: normalizeModel(talkProviderConfig.modelId) },
        ...talkProviderConfig.latency == null ? {} : { latency: normalizeLatency(talkProviderConfig.latency) },
        ...asNumber(talkProviderConfig.speed) == null ? {} : { speed: asNumber(talkProviderConfig.speed) },
        ...asNumber(talkProviderConfig.temperature) == null ? {} : { temperature: asNumber(talkProviderConfig.temperature) },
        ...asNumber(talkProviderConfig.topP) == null ? {} : { topP: asNumber(talkProviderConfig.topP) }
      };
    },
    ...
    listVoices: async (req) => {
      const config = req.providerConfig ? readFishAudioProviderConfig(req.providerConfig) : void 0;
    ...
    synthesize: async (req) => {
      ...
      return {
        ...
        // Only voice-note targets (Telegram, WhatsApp, etc.) get voice-compatible output
      };
    }
  };
}
export {
  buildFishAudioSpeechProvider,
